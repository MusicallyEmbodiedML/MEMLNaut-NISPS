...
void MLP<T>::CreateMLP(const std::vector<size_t> & layers_nodes,
    ...
    // NOTE: was `assert(loss_mgr.GetLossFunction(...))`. GetLossFunction has
    // the side effect of populating loss_fn_; wrapping it directly in
    // assert() means a standard Release build (-DNDEBUG, which strips
    // assert()) never calls it at all, leaving loss_fn_ null and crashing on
    // the first Train()/TrainBatch() call. Call it unconditionally and keep
    // the assert purely as a debug-build sanity check.
    const bool have_loss_fn = loss_mgr.GetLossFunction(loss_function, &(this->loss_fn_));
    assert(have_loss_fn);
    (void)have_loss_fn;
